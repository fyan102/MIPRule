...
def evaluate(dataset_name, load_method, feature_map={}, loss='squared',
    ...
    for m in range(repeat):
        ...
        output = open(
            "../output20220810realkd_cv/" + dataset_name + "_realkd_col" + str(col) + str(datetime.now()) + ".txt", "a")
...
if __name__ == '__main__':
    ...
    for col in [10]:  # finished
        try:
            res['load_wine'] = evaluate('load_wine', load_wine, feature_map={'target': {0: -1, 1: 1, 2: -1}},
                                        repeat=5, regs=[0.1, 0.2, 0.5, 0.7, 1, 2, 4, 8, 16], loss='logistic',
                                        max_rule_num=10, col=col)
        except Exception as e:
            print("Error 1", e)
    for col in [10]:  # finished
        try:
            res['iris'] = evaluate('iris', load_iris, feature_map={'target': {0: -1, 1: 1, 2: -1}},
                                   repeat=5, regs=[0.1, 0.2, 0.5, 0.7, 1, 2, 4, 8, 16], loss='logistic',
                                   max_rule_num=10, col=col)
        except Exception as e:
            print("Error 1", e)
    for col in [5]: # finished
        try:
            res['diabetes'] = evaluate('load_diabetes', load_diabetes,
    ...
    for col in [5]:
        try:
            res['breast'] = evaluate('breast_cancer', load_breast_cancer,
                                     repeat=5, regs=[0.1, 0.2, 0.5, 0.7, 1, 2, 4, 8, 16], loss='logistic',
                                     max_rule_num=10, col=col)
        except Exception as e:
            print("Error 1", e)
