...
def evaluate(dataset_name, path, labels, feature_types, target, target_type=int, feature_map={}, loss='squared',
    ...
    for m in range(repeat):
        ...
        #                                                                        'discretization': qcut,
        #                                                                        'max_col_attr': col}))
        #     scores[r] = cv(train, train_target, boosting, labels, method='b', loss=loss)R
        # print(scores)
        # # find best lambda
...
if __name__ == '__main__':
    ...
    for col in [10]:  # finished
        try:
            res['world_happiness_indicator'] = evaluate('world_happiness_indicator',
    ...
    for col in [2]:
        try:
            res['Demographics'] = evaluate('Demographics', '../datasets/Demographics/Demographics1.csv',
                                           ['Sex', 'Marital', 'Age', 'Edu', 'Occupation', 'LivingYears', 'Persons',
                                            'PersonsUnder18', 'HouseholderStatus',
                                            'TypeOfHome', 'Ethnic', 'Language'],
                                           [str, str, int, int, str, int, int, int, str, str, str, str], 'AnnualIncome',
                                           target_type=int,
                                           feature_map={'Sex': {' Male': 1, ' Female': 0},
                                                        'Marital': {' Married': 1, '': 0, ' Single, never married': 2,
                                                                    ' Divorced or separated': 3,
                                                                    ' Living together, not married': 4, ' Widowed': 5},
                                                        ...
                                                        'Language': {'': 0, ' English': 1, ' Spanish': 2,
                                                                     ' Other': 3, }
                                                        }, regs=[0, 0.1, 0.2, 0.5, 0.7, 1, 2, 4, 8, 16],
                                           repeat=5, max_rule_num=10, col=col)
        except Exception as e:
            print("Error 1", e)
    ...
    # for col in [4]:
    #     try:
    #         res['mobile_prices'] = evaluate('mobile_prices',
    #                                         '../datasets/mobile_prices/train.csv',
    #                                         ['battery_power', 'blue', 'clock_speed', 'dual_sim', 'fc', 'four_g',
    #                                          'int_memory', 'm_dep', 'mobile_wt', 'n_cores', 'pc', 'px_height',
    #                                          'px_width', 'ram', 'sc_h', 'sc_w', 'talk_time', 'three_g', 'touch_screen',
    #                                          'wifi'],
    #                                         [int, int, float, int, int, int, int, float, int, int, int, int, int, int,
    #                                          int,
    #                                          int, int, int, int, int, ], 'price_range',
    #                                         target_type=int, regs=[0, 0.1, 0.2, 0.5, 0.7, 1, 2, 4, 8, 16],
    #                                         repeat=5, max_rule_num=10, col=col)
    #     except Exception as e:
    #         print("Error 1", e)
    for col in [3]:
