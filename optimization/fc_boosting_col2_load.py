...
def fully_corrective2(n, d, k, L, U, x, y, labels, init_w, init_l, init_u, reg=0, loss_func='squared', tl=100, f=None,
                      max_col_num=10):
    m = gp.Model("rule1")
...
def fully_corrective_boosting2(n, d, k, L, U, x, y, labels, reg=0, loss_func='squared', tl=100, f=None, left_most=0,
...
def evaluate_fc_boosting2(dataset_name, load_method, cr='r', feature_map={},
                          loss='squared', tl=500, repeat=5, max_rule_num=5, debug=False, reg=0, max_col_num=10):
    seeds = get_splits()[dataset_name]
    k = max_rule_num
    func = skl_auc if cr == 'c' else skl_r2
    file = open(
        "../output20220524fc2/" + dataset_name + str(
            datetime.now()) + "_fc_boosting2_no_priority_sym_ind_950_reg" + str(
            reg) + "tl" + str(tl) + 'col' + str(max_col_num) + ".txt", "w")
    original_stdout = sys.stdout
    with open("../output20220524fc2/" + dataset_name +
              str(datetime.now()) + "_output_fc_boosting2_no_priority_sym_ind_950_reg" + str(reg) + "tl" + str(
