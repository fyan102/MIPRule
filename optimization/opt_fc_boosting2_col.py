...
def evaluate_fc_boosting(dataset_name, path, labels, feature_types, target, target_type, cr='r', feature_map={},
                         loss='squared', tl=2500, repeat=5, max_rule_num=5, debug=False, reg=0, max_col_num=10):
    seeds = get_splits()[dataset_name]
    k = max_rule_num
    func = skl_auc if cr == 'c' else skl_r2
    for m in range(repeat):
        file = open(
            "../output20241024opt2/" + dataset_name + "_opt2_no_priority_sym_ind_950_reg" + str(
                reg) + "tl" + str(
                tl) + 'col' + str(max_col_num) + 'rep' + str(m) + ".txt", "w")
        original_stdout = sys.stdout
        with open("../output20241024opt2/" + dataset_name + "_output_opt2_no_priority_sym_ind_950_reg" + str(
                reg) + "tl" + str(
